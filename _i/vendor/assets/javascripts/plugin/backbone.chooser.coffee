do (Backbone) ->
  ...
  class BaseChooser
    ...
    _publicMethods: ->
      ["choose", "unchoose", "getChosen", "getFirstChosen", "chooseById", "chooseNone"]
  ...
  class Backbone.MultiChooser extends BaseChooser
    constructor: ->
      super
      @collection[method] = _.bind(@[method], @) for method in ["chooseAll", "chooseNone",
                                                                "chooseByIds"]
    ...
    choose: (args...) ->
      options =
        if _.chain(args).flatten().last().value() not instanceof Backbone.Model
          args.pop()
        else
          {}
    ...
    _getEvent: ->
      # AMN переместил вверх, так если удалить все модели из коллекции,
      # то должно быть сообщение collection:chose:none, а не collection:chose:all
